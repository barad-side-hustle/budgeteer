...
**Not running as Administrator?** Setup will continue, but skip the hosts entry. `http://127.0.0.1:2412` still works; `http://budgeteer.local:2412` won't resolve. Re-run setup from an elevated shell later to add the friendly hostname.
</Callout>
...
3. **Registers a Task Scheduler logon trigger** that runs `next start -H 127.0.0.1 -p 2412` whenever you log in. Server is only reachable from loopback.
