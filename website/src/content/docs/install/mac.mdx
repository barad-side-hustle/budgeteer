...
```sh
git clone https://github.com/barad-side-hustle/budgeteer.git ~/budgeteer
cd ~/budgeteer
```
...
2. **Adds `127.0.0.1 budgeteer.local` to `/etc/hosts`** so you can reach the dashboard at `http://budgeteer.local:2412`. Asks for `sudo` once.
3. **Registers a LaunchAgent** at `~/Library/LaunchAgents/io.budgeteer.app.plist` that starts the server on login. Server binds to `127.0.0.1:2412` only.
