...
declare global {
  var __budgeteerActivity: ActivityState | undefined;
}

const STALE_MS = 5 * 60 * 1000;

function getState(): ActivityState {
  if (!globalThis.__budgeteerActivity) {
    globalThis.__budgeteerActivity = {
      syncStartedAt: null,
      syncKind: null,
      syncDepth: 0,
  ...
  return globalThis.__budgeteerActivity;
}
...
export function getActivitySnapshot(): ActivitySnapshot {
  ...
  return {
    ...
    ollama: {
      ...
      spawnedByBudgeteer: ollamaSpawned,
    },
  };
}
