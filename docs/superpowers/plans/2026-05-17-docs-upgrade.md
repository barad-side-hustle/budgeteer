...
**Goal:** Rebuild the Budgeteer docs at `website/src/content/docs/` so they match the homepage vibe, serve both technical and non-technical readers via a plain-English + `<details>` pattern, and reflect the current `npm install` + `npm run setup` install flow.

**Architecture:** A small in-doc Astro component library under `website/src/components/docs/` provides the visual vocabulary (hero, card grid, callouts, step list, fragment components extracted from the homepage). Each doc page is rewritten to use those components. A new script `scripts/capture-docs-screenshots.mjs` seeds a tmp fake-data DB, boots `next dev` pointed at it via a new `BUDGETEER_DATA_DIR` env override, walks the dashboard with Puppeteer, and writes the screenshots into `website/src/assets/screenshots/`. The Starlight sidebar in `website/astro.config.mjs` is reshaped around the user journey.

**Tech Stack:** Astro 5 + Starlight 0.37, MDX, Puppeteer (transitively present), Next.js 16, better-sqlite3, existing Budgeteer design tokens in `website/src/styles/global.css`.
...
- `website/src/content/docs/what-is-budgeteer.mdx`
...
- `src/server/db/index.ts` — accept `BUDGETEER_DATA_DIR` env var override (1-line change to enable capture script)
...
- Page slugs match file paths under `src/content/docs/`. Internal links use `/budgeteer/<slug>` (the configured `base`).
...
## Task 1: Add `BUDGETEER_DATA_DIR` env override
...
Run: `BUDGETEER_DATA_DIR=/tmp/spent-smoke npm run dev` (Ctrl-C after ~5s once "Ready" appears).
Expected: server starts. `/tmp/spent-smoke/budgeteer.db` exists.
...
git commit -m "feat(db): allow BUDGETEER_DATA_DIR override for tooling"
```
...
Open: `http://localhost:4321/budgeteer/getting-started`
...
<DocsCardGrid>
  <DocsCard eyebrow="INSTALL" title="On macOS" desc="Three commands, ten minutes." href="/budgeteer/install/mac" accent="vibrant" />
  <DocsCard eyebrow="USING" title="Connect a bank" desc="What each bank needs." href="/budgeteer/connect-bank" accent="pink" />
  <DocsCard eyebrow="USING" title="Categorize with AI" desc="Claude, Ollama, or manual." href="/budgeteer/ai-categorization" accent="blue" />
</DocsCardGrid>
```
...
<NextStepCard href="/budgeteer/install/mac" title="Install on macOS" />
```
...
sidebar: [
  {
    ...
    items: [
      ...
      { label: 'What is Budgeteer?', slug: 'what-is-budgeteer' },
    ],
  },
  {
    label: 'Install',
...
Run: dev server. Visit `/budgeteer/troubleshooting`, `/budgeteer/security-and-privacy`, `/budgeteer/disclaimer`. All three render the same as before.
...
Spins up Next.js against a tmp `BUDGETEER_DATA_DIR`, seeds the DB via the existing app boot (which runs migrations), inserts fake data via raw SQL, captures the screenshots, then cleans up.
...
function seedDb(dbDir) {
  ...
  const dbPath = path.join(dbDir, 'budgeteer.db');
...
function startServer(dataDir) {
  const env = { ...process.env, BUDGETEER_DATA_DIR: dataDir, PORT: String(PORT) };
...
Run: dev server. Open `/budgeteer/getting-started`. You should see the new hero, intro paragraph, dashboard screenshot, gotcha callout, and a 9-card grid. Click each card — they should land on the right pages (some 404 until later tasks).
...
## Task 18: Write `what-is-budgeteer.mdx`

**Files:**
- Create: `website/src/content/docs/what-is-budgeteer.mdx`

- [ ] **Step 1: Write the page**

```mdx
---
title: What is Budgeteer?
...
## Why Budgeteer exists

Israeli banks have terrible exports. YNAB doesn't speak ILS gracefully. Every "cloud finance" app wants you to hand over your bank password to a server you don't control. Budgeteer is the answer for people who'd rather just run something on their own laptop.
...
<Callout variant="gotcha">
Budgeteer is **read-only**. It cannot transfer funds, pay bills, or change settings inside your bank account. It only reads transaction data the bank's website already shows you.
</Callout>
...
**Beta.** Built by [Shay Avivi](https://github.com/Shaya16) for personal use first and then released. MIT licensed. Issues and PRs welcome on [GitHub](https://github.com/barad-side-hustle/budgeteer).
...
Open `/budgeteer/what-is-budgeteer`. Verify hero, paragraphs, gotcha callout, and a sage "Next" card pointing to mac install.

- [ ] **Step 3: Commit**

```bash
git add website/src/content/docs/what-is-budgeteer.mdx
git commit -m "feat(docs): add What is Budgeteer? intro page"
```
...
<DocsHero
  ...
  lede="Three commands, about ten minutes. Budgeteer will install itself as a background service and put a small icon in your menu bar."
/>
...
Budgeteer runs on Node.js, a small runtime you install once. Go to **[nodejs.org](https://nodejs.org)**, download the **LTS** installer for macOS, and run it. Click through the defaults.
...
```sh
git clone https://github.com/barad-side-hustle/budgeteer.git ~/Applications/Budgeteer
cd ~/Applications/Budgeteer
```

No Git? Download the [latest source as a zip](https://github.com/barad-side-hustle/budgeteer/releases) and unzip it to `~/Applications/Budgeteer`. Then in Terminal: `cd ~/Applications/Budgeteer`.
...
2. **Adds `127.0.0.1 budgeteer.local` to `/etc/hosts`** so you can reach the dashboard at `http://budgeteer.local:41234`. Asks for `sudo` once.
3. **Registers a LaunchAgent** at `~/Library/LaunchAgents/io.budgeteer.app.plist` that starts the server on login. Server binds to `127.0.0.1:41234` only.
4. **Builds `Budgeteer.app`** from `menubar/mac/` (Swift). Needs Xcode Command Line Tools (offered for install if missing).
5. **Copies `Budgeteer.app` to `~/Applications/`** and registers it as a Login Item so the menubar shows up after reboot.
...
<Callout variant="gotcha">
*"Budgeteer.app can't be opened because it is from an unidentified developer."* — Right-click `Budgeteer.app` in `~/Applications`, choose **Open**, then **Open** in the dialog. After the first launch, double-click works normally.
</Callout>
...
Open `/budgeteer/install/mac`. Verify hero, three step cards with pastel-rotation numbers, the auto-Xcode callout, the For Developers accordion, the Gatekeeper gotcha, day-to-day commands, uninstall, and the Next CTA.
...
When the installer asks *"Automatically install the necessary tools"* near the end, **leave that box unchecked** — Budgeteer's setup will handle the right build tools for you.
...
```powershell
git clone https://github.com/barad-side-hustle/budgeteer.git $env:USERPROFILE\Budgeteer
cd $env:USERPROFILE\Budgeteer
```

No Git? Download the [latest source zip](https://github.com/barad-side-hustle/budgeteer/releases), right-click → *Extract All* into `%USERPROFILE%\Budgeteer`, then `cd` there.
...
<Callout variant="gotcha">
**Not running as Administrator?** Setup will continue, but skip the hosts entry. `http://127.0.0.1:41234` still works; `http://budgeteer.local:41234` won't resolve. Re-run setup from an elevated shell later to add the friendly hostname.
</Callout>
...
1. **Builds the Next.js app** (`npm run build`).
2. **Adds `127.0.0.1 budgeteer.local` to `C:\Windows\System32\drivers\etc\hosts`** (skipped if not elevated).
...
4. **Builds `Budgeteer.exe`** from `menubar/windows/` (C#, .NET 8). Auto-installs the SDK via `winget install Microsoft.DotNet.SDK.8` if missing.
5. **Copies `Budgeteer.exe` to `%LOCALAPPDATA%\Programs\Budgeteer\`** and drops a `.lnk` in `shell:startup` so the tray app shows up after reboot.
...
Open `/budgeteer/install/windows`. Verify the three-step structure, both callouts (note + gotcha), For Developers details, day-to-day commands, Next CTA.
...
<Callout variant="note">
**No menubar on Linux.** The setup script installs the service only. You'll open the dashboard at `http://127.0.0.1:41234` (or `http://budgeteer.local:41234` if you add the hosts entry manually with `sudo`).
</Callout>
...
Open `/budgeteer/install/linux`. Verify three steps, note callout, dev details, commands, uninstall, Next CTA.
...
Open `/budgeteer/connect-bank`. Verify hero, bank form fragment, the per-bank credential reference, the gotcha for Yahav, the sync progress fragment, and the Next CTA.
...
description: How Budgeteer uses AI to sort transactions, and how to pick a provider.
---
...
<DocsHero
  ...
  titleItalic="with AI."
  lede="Budgeteer assigns each transaction to a category like Groceries, Restaurants, Transport, or Subscriptions. You pick which model does the work."
/>
...
Cost-effective for personal use because of Claude Haiku 4.5's pricing and Budgeteer's batching (50 transactions per call).

**Setup:**

1. Go to `console.anthropic.com` and create an API key under *Settings → API Keys*.
2. Paste it into Budgeteer's setup wizard (or *Settings → AI*).
3. Done. Your key is encrypted at rest with the same AES-256-GCM that protects your bank credentials.

<Callout variant="note">
Budgeteer sends Anthropic's API the **merchant name and amount** of each new transaction — no personal IDs, no bank credentials. Anthropic's commercial API does not train on submitted data.
</Callout>
...
3. In Budgeteer's setup wizard, choose Ollama and pick the model.

Budgeteer talks to Ollama at `http://localhost:11434`, its default.
...
When Budgeteer syncs, it groups uncategorized transactions into batches of **50** and sends each batch to the chosen provider in one round-trip. This keeps cost and latency low.
...
Budgeteer retries up to **three times** on malformed output. Persistent failures fall back to *Uncategorized* — never to a wrong category.
...
<Callout variant="gotcha">
**If you switch away from Claude, revoke your API key** in the Anthropic console. Budgeteer will discard it locally, but rotating it is the safe move.
</Callout>

<NextStepCard href="/budgeteer/sync-and-dashboard" title="Sync & dashboard" />
```

- [ ] **Step 2: Verify**

Open `/budgeteer/ai-categorization`. Verify hero, transaction list fragment, three provider cards, per-provider sections with anchors (`#claude`, `#ollama`, `#manual`) navigable from the cards.
...
Visit `http://localhost:41234` (or `http://budgeteer.local:41234` if you added the hosts entry). The dashboard has four areas:
...
Open `/budgeteer/sync-and-dashboard`. Confirm three screenshots load (light dashboard, transactions, dark dashboard), the sync fragment renders, all sections present.
...
description: How Budgeteer groups your spending, sets targets, and detects transfers.
---
...
<DocsHero
  ...
  lede="How Budgeteer groups your spending, where the colors come from, and how 'ahead of pace' is calculated."
/>

## The category set

Budgeteer ships with 16 curated categories:
...
Click any category badge in the transactions list to change it. Budgeteer remembers the correction and applies it the next time it sees a similar merchant.

## Merchant memory

Once you correct an AI categorization, Budgeteer caches the merchant-to-category mapping. The next batch of transactions from the same merchant goes straight to the right category without another call to the AI.

This is how Budgeteer stays cheap on Claude — most ongoing sync calls only categorize *new* merchants.
...
Credit card payments (e.g. an Isracard bill paid from your Hapoalim account) show up twice — once as the card statement, once as the bank withdrawal. Budgeteer's matching pass detects these and categorizes them as *Transfers* so they're excluded from spending totals.

The same logic handles inter-account moves between two banks you've connected.

<Callout variant="note">
If you spot a real expense miscategorized as a transfer, re-categorize it manually. Budgeteer will not "correct itself" in the other direction — your manual choice wins.
</Callout>

<NextStepCard href="/budgeteer/hebrew-and-rtl" title="Hebrew & RTL" />
```

- [ ] **Step 2: Verify**

Open `/budgeteer/categories-and-budgets`. Verify hero, category list, callouts, Next CTA.
...
---
title: Hebrew & RTL
description: Switching Budgeteer to Hebrew and how the right-to-left layout works.
---
...
<DocsHero
  ...
  titleBold="Hebrew"
  titleItalic="& RTL."
  lede="Budgeteer's UI is available in Hebrew with full right-to-left layout. Switch any time."
/>
...
If any of these block you, please file an issue on [GitHub](https://github.com/barad-side-hustle/budgeteer/issues).

<NextStepCard href="/budgeteer/troubleshooting" title="Troubleshooting" />
```

- [ ] **Step 2: Verify**

Open `/budgeteer/hebrew-and-rtl`. Verify hero, sections, callouts, Next CTA.
...
<Callout variant="gotcha">
*"MSB8020"* or other native-compile errors — Budgeteer depends on **better-sqlite3**, which compiles natively. The `npm run setup` script handles the build-tool prompt; if you ran `npm install` from a non-elevated PowerShell first, the compile may not have been able to register the .NET 8 SDK.
</Callout>
...
### "Budgeteer.app is from an unidentified developer" (macOS)

Right-click `Budgeteer.app` in `~/Applications`, choose **Open**, then **Open** in the dialog. After the first launch, double-click works.
...
- Your bank requires 2FA, which Budgeteer can't do (except One Zero). Disable 2FA on the bank's side.
- The bank's website is down. Try again in an hour.

### Sync hangs or times out

The bank's website is being slow. Budgeteer waits up to **5 minutes** per login. If it consistently times out, increase the timeout in *Settings → Advanced* or sync at a different time of day.
...
Some banks expose limited history. Yahav, in particular, only goes back about **6 months**. Older data isn't reachable via the website Budgeteer scrapes.
...
In `data/budgeteer.db` (a SQLite file) and `data/.encryption-key`. Both live inside your Budgeteer install folder.
...
### How do I move Budgeteer to a new computer?

1. Install Budgeteer on the new machine ([macOS](/budgeteer/install/mac) / [Windows](/budgeteer/install/windows) / [Linux](/budgeteer/install/linux)).
2. Before the first run, copy `data/budgeteer.db` **and** `data/.encryption-key` from the old computer into the new install's `data/` folder.
3. Start the service. Budgeteer picks up where it left off.

<ForDevelopers summary="Backing up via SQLite tools">

`data/budgeteer.db` is a standard SQLite database in WAL mode. Any tool that respects WAL works:

```sh
sqlite3 data/budgeteer.db ".backup data/spent-backup.db"
```
...
Open an issue on [GitHub](https://github.com/barad-side-hustle/budgeteer/issues) with:
...
Open `/budgeteer/troubleshooting`. Verify hero, four section groups, callouts where added, ForDevelopers accordion at the bottom.
...
description: The full story on how Budgeteer handles your credentials, transactions, and AI queries.
---
...
Budgeteer is built to be the kind of finance app you'd actually trust with your bank password. That means a clear story.

## What Budgeteer stores, and where

Everything Budgeteer saves lives inside the `data/` folder of your install directory. Two files:

- **`data/budgeteer.db`** — a SQLite database with your encrypted bank credentials, transactions, categories, and settings.
- **`data/.encryption-key`** — a 32-byte random key generated on first run.

Both files are local. Budgeteer never uploads them. There is no "Budgeteer cloud."
...
- If someone copies just `data/budgeteer.db` off your computer, they cannot read your passwords.
...
1. **Bank logins** — Budgeteer opens a headless Chromium tab and logs into your bank's website using the credentials you provided. This traffic goes directly between your computer and your bank.
...
3. **No analytics, no telemetry, no crash reports.** Budgeteer does not phone home. Ever.
...
In practice, this is rare for read-only scraping that happens infrequently. But it's a real risk and you should know about it. Read the [Disclaimer](/budgeteer/disclaimer) for the full version.
...
For non-security bugs, [GitHub Issues](https://github.com/barad-side-hustle/budgeteer/issues) is the right place.

## Auditing the code yourself

Budgeteer is open source. Relevant files:
...
<NextStepCard href="/budgeteer/disclaimer" title="Disclaimer" />
```

- [ ] **Step 2: Verify**

Open `/budgeteer/security-and-privacy`. Verify hero, two paired callouts under the threat model, Next CTA.
...
Open `/budgeteer/disclaimer`. Verify hero and the final accept-terms callout.
...
- `/budgeteer/getting-started`
- `/budgeteer/what-is-budgeteer`
- `/budgeteer/install/mac`
- `/budgeteer/install/windows`
- `/budgeteer/install/linux`
- `/budgeteer/connect-bank`
- `/budgeteer/ai-categorization`
- `/budgeteer/sync-and-dashboard`
- `/budgeteer/categories-and-budgets`
- `/budgeteer/hebrew-and-rtl`
- `/budgeteer/troubleshooting`
- `/budgeteer/security-and-privacy`
- `/budgeteer/disclaimer`
...
- What is Budgeteer? → Install on macOS
